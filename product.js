const loadApi = () => {
    ...
    const active_button_all = document.querySelectorAll(".active_button");
    active_button_all.forEach(btn => btn.classList.remove('btn-active'));
    const active_button = document.getElementById('all_active');

    if (active_button) {
        active_button.classList.add('btn-active');
    }
}

const loadfilture = (catagory) => {
    const url = `https://fakestoreapi.com/products/category/${encodeURIComponent(catagory)}`;
    fetch(url)
        .then(res => res.json())
        .then(data => displayAllProduct(data));
    ...
    // Create same formatted id
    const formattedId = `btn-${catagory.toLowerCase().replace(/[^a-z0-9]/g, '-')}`;

    const active_button = document.getElementById(formattedId);
...
const loadCetagory = () => {
    ...
    fetch(url)
        .then(res => res.json())
        .then(data => displayCetagory(data));
...
const displayCetagory = (data) => {
    const catagoryButton = document.getElementById('catagoryButton');
    ...
    data.forEach(category => {
        ...
        button.id = `btn-${category.toLowerCase().replace(/[^a-z0-9]/g, '-')}`;
        button.className =
            "btn active_button btn-outline btn-primary border-base-300 text-base-content btn-sm rounded-full px-6 capitalize";

        button.addEventListener("click", () => {
            loadfilture(category);
        });
...
const addToCartItem = (item) => {
    ...
    cardItemList.innerHTML = `
                <div id="cart_item_${item.id}" class="flex gap-4 items-center bg-base-200/50 p-3 rounded-xl mb-2">
